...
const comment = P.optWhitespace.then(P.regex(/\/\/.*/)).then(P.end).skip(P.optWhitespace);
...
const parseOp = ops => {
    const find = (arr, i) => (i >= ops.length) ? P.fail('') : operator(arr[i]).or(find(arr, i + 1));
    return find(ops, 0); 
}
...
const intMatch = P.regexp(/[+-]?[0-9]+/).notFollowedBy(P.regexp(/[.eE]/))
    .desc('integer').skip(ws);

// Matches float numbers and infinity.
const floatMatch = P.alt(P.regexp(/[+-]?([0-9]+\.?[0-9]*|\.[0-9]+)([eE][+-]?[0-9]+)?/), P.string('Infinity'), P.string('-Infinity'))
    .desc('number').skip(ws);
...
const num = lineNum.chain(line => intMatch.map(str => a.integer(str, line)).or(floatMatch.map(str => a.float(str, line))));

// Parser that matches ASCII characters 0 or more times between " " only.
...
const none = lineNum.chain(line => P.string('null').desc('null').skip(ws).map(_ => a.null_(line)));
...
const collection = P.lazy(() => lineNum.chain(line => 
    P.seq(P.alt(stringMatch, intMatch, floatMatch, name).skip(operator(':')), expr).sepBy(operator(','))
...
const reference_or_subscription = P.lazy(() => lineNum.chain(line => atom.chain(lhs => P.alt(P.seqObj(['attribute', token('.').then(name)])
        , P.seqObj(['subscriptor', expr.wrap(operator('['), operator(']'))])).atLeast(1).map(seqObj => 
            seqObj.reduce((acc, currVal) => {
                if(currVal.attribute !== undefined) {
                    return a.attribute(acc, currVal.attribute, line);
                } else {
                    return a.subscriptor(acc, currVal.subscriptor, line);
                }
